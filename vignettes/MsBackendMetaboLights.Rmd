...
```{r, echo = FALSE}
#' Sleep for random number of seconds; only needed to ensure daily unit
#' tests from Bioconductor don't fail because of rate limitations of\
...
fail. We use below the `retry()` function from the `r Biocpkg("MsCoreUtils")`
package to retry reading from the FTP server if the connection fails or gets
closed before the data is fully read.
...
Importantly, as can be seen above, the MS data files are loaded in **alphabetic order**.
...
The MetaboLights-specific variables are `"mtbls_id"`, `"mtbls_assay_name"`,
`"mtbls_assay_id"` and `"derived_spectral_data_file"` providing the
MetaboLights ID of the data set, the assay/method with which the data files
were generated and the original file path/name of the data files on the
MetaboLights ftp server.
...
Next to the code to cache and download MS data files, *MsBackendMetaboLights*
provides also utility functions to retrieve general data and information for a
data set from the MetaboLights repository. These functions are listed on the
`?Metabolites-utils` help page. The `mtbls_list_files()` for example allows to
list all available files in the data set's base FTP folder.

```{r}
mtbls_list_files("MTBLS39")
```

The `mtbls_assay_data()` retrieves information on the, or one of the possibly
multiple, used assay(s).
...
The `mtbls_sample_data()` retrieves the sample information for a data set.
...
And the `mtbls_metadata()` combines the sample and assay information into a
single `data.frame` from which individual sample information for the respective
MS data files could be extracted.
