# Authors and Citation

## Authors

- **Johannes Rainer**. Author, maintainer.
  [](https://orcid.org/0000-0002-6977-7147)

- **Philippine Louail**. Author.
  [](https://orcid.org/0009-0007-5429-6846)

- **Gabriele Tomè**. Author. [](https://orcid.org/0000-0002-3976-6068)\
  MetaRbolomics4Galaxy project (CUP: D53C25001030003) co-funded by the
  Autonomous Province of Bolzano under the Joint Projects South
  Tyrol–Germany 2025 program.

## Citation

Source:
[`DESCRIPTION`](https://github.com/RforMassSpectrometry/MsBackendMetaboLights/blob/main/DESCRIPTION)

Rainer J, Louail P, Tomè G (2026). *MsBackendMetaboLights: Retrieve Mass
Spectrometry Data from MetaboLights*. R package version 1.7.5,
<https://github.com/RforMassSpectrometry/MsBackendMetaboLights>.

@Manual{, title = {MsBackendMetaboLights: Retrieve Mass Spectrometry
Data from MetaboLights}, author = {Johannes Rainer and Philippine Louail
and Gabriele Tomè}, year = {2026}, note = {R package version 1.7.5}, url
= {https://github.com/RforMassSpectrometry/MsBackendMetaboLights}, }
